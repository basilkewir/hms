        ...
        <div v-if="selectedGuest" class="shadow rounded-lg p-6"
            ...
            <form @submit.prevent="processCheckOut" class="space-y-6">
                    ...
                    <div class="rounded-lg p-4 space-y-4 border"
                        ...
                        <div>
                            <h5 class="font-semibold mb-2"
                                :style="{ color: themeColors.textPrimary }">Room Charges</h5>
                            <div class="space-y-2">
                                <div class="flex justify-between text-sm"
                                    :style="{ color: themeColors.textSecondary }">
                                    <span>Room</span>
                                    <span :style="{ color: themeColors.textPrimary }">{{ selectedGuest.roomNumber }}</span>
                                </div>
                                <div class="flex justify-between text-sm"
                                    :style="{ color: themeColors.textSecondary }">
                                    <span>Rate per night</span>
                                    <span :style="{ color: themeColors.textPrimary }">{{ formatCurrency(parseFloat(selectedGuest.room_rate?.replace(/,/g, '') || 0)) }}</span>
                                </div>
                                <div class="flex justify-between text-sm"
                                    :style="{ color: themeColors.textSecondary }">
                                    <span>Nights stayed: {{ selectedGuest.actual_nights || selectedGuest.nights }}</span>
                                    <span v-if="selectedGuest.is_early_checkout" class="text-xs"
                                        :style="{ color: themeColors.textTertiary }">
                        ...
                        <div v-if="selectedGuest.posSales && selectedGuest.posSales.length > 0">
                            <h5 class="font-semibold mb-2"
                                :style="{ color: themeColors.textPrimary }">Restaurant & Bar Charges</h5>

                            <!-- Unpaid Bills Warning -->
                            <div v-if="selectedGuest.hasUnpaidBills" class="rounded-lg p-3 mb-4"
                                 :style="{
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium"
                                           :style="{ color: '#ef4444' }">Unpaid Bills Detected</p>
                                        <p class="text-xs mt-1"
                            ...
                            <div class="space-y-2 mb-2">
                                <div v-for="sale in selectedGuest.posSales" :key="sale.id"
                                    ...
                                    <div class="flex justify-between items-center border-b pb-1 mb-1"
                                        ...
                                        <div class="flex items-center gap-2">
                                            <span class="text-xs font-medium"
                                                  :style="{ color: themeColors.textSecondary }">Sale #{{ sale.sale_number }}</span>
                                        ...
                                        <span class="text-xs"
                                              :style="{ color: themeColors.textTertiary }">{{ sale.sale_date }}</span>
                                    </div>
                                    <div class="space-y-1">
                                        <div v-for="(item, idx) in sale.items" :key="idx"
                                    ...
                                    <div class="flex justify-between font-semibold text-xs pt-1 border-t mt-1"
                                         :style="{
                                             borderColor: themeColors.border,
                                             color: themeColors.textPrimary
                                         }">
                        ...
                        <div v-if="selectedGuest.folio && selectedGuest.folio.charges && selectedGuest.folio.charges.filter(c => c.charge_code === 'SERVICE').length > 0">
                            <h5 class="font-semibold mb-2"
                                :style="{ color: themeColors.textPrimary }">Service Charges</h5>
                            <div class="space-y-1 mb-2">
                                <div v-for="charge in selectedGuest.folio.charges.filter(c => c.charge_code === 'SERVICE')"
                                     :key="charge.id"
                                     class="flex justify-between text-sm rounded p-2"
                                     :style="{
                                         backgroundColor: themeColors.card,
                                         color: themeColors.textSecondary
                                     }">
                                    <div>
                                        <span :style="{ color: themeColors.textPrimary }">{{ charge.description }}</span>
                                        <span class="text-xs ml-2"
                                              :style="{ color: themeColors.textTertiary }">{{ charge.charge_date }} {{ charge.charge_time || '' }}</span>
                                    </div>
                                    <span :style="{ color: themeColors.textPrimary }">{{ formatCurrency(charge.net_amount) }}</span>
                                </div>
                            </div>
                            <div class="flex justify-between font-medium pt-2 border-t"
                                :style="{
                        ...
                        <!-- Summary -->
                        <div class="pt-4 border-t-2 space-y-2"
                             :style="{ borderColor: themeColors.border }">
                            ...
                            <div v-if="parseFloat(selectedGuest.discountAmount?.replace(/,/g, '') || 0) > 0" class="flex justify-between"
                                :style="{ color: '#22c55e' }">
                                <span>Discount</span>
                            ...
                            <div v-if="parseFloat(selectedGuest.unifiedBalance?.replace(/,/g, '') || selectedGuest.balanceAmount?.replace(/,/g, '') || 0) > 0"
                                 class="flex justify-between font-semibold mt-2 pt-2 border-t"
                                 :style="{
                                     borderColor: themeColors.border,
                                     color: '#ef4444'
                                 }">
                            ...
                            <div v-else class="flex justify-between font-semibold mt-2 pt-2 border-t"
                                 :style="{
                                     borderColor: themeColors.border,
                                     color: '#22c55e'
                                 }">
                ...
                <div>
                    ...
                    <div v-if="checkOutForm.damages && checkOutForm.damages.length" class="space-y-2 mb-2">
                        <div v-for="(damage, index) in checkOutForm.damages" :key="index"
                             class="flex flex-col md:flex-row md:items-center gap-2 rounded-lg p-3 border"
                             :style="{
                                 backgroundColor: themeColors.background,
                                 borderColor: themeColors.border
                             }">
                            ...
                            <button type="button"
                                    ...
                                    class="self-start md:self-center text-xs"
                                    :style="{ color: themeColors.danger }">
                                Remove
                            </button>
                        </div>
                    </div>
                    <p v-if="damageTotal > 0" class="text-xs"
                       :style="{ color: themeColors.textSecondary }">
                        Damage total to be added at checkout:
                        <span class="font-semibold"
                              :style="{ color: themeColors.danger }">{{ formatCurrency(damageTotal) }}</span>
                    </p>
                </div>
                ...
                <div v-if="selectedGuest.key_card">
                    <h4 class="text-md font-medium mb-4"
                        :style="{ color: themeColors.textPrimary }">Key Card Return</h4>
                    <div class="rounded-lg p-4 mb-4 border"
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="font-medium"
                                   :style="{ color: themeColors.textPrimary }">Key Card: {{ selectedGuest.key_card.card_number }}</p>
                                <p class="text-sm"
                                   :style="{ color: themeColors.textSecondary }">Type: {{ selectedGuest.key_card.card_type }}</p>
                            </div>
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-2"
                               :style="{ color: themeColors.textSecondary }">Key Card Status</label>
                        ...
                        <p class="text-xs mt-1"
                           :style="{ color: themeColors.textTertiary }">Select the status of the key card return</p>
                    </div>
                </div>
                ...
                <div>
                    <h4 class="text-md font-medium mb-4"
                        :style="{ color: themeColors.textPrimary }">Payment Settlement</h4>

                    <!-- Warning if unpaid bills exist -->
                    <div v-if="selectedGuest.hasUnpaidBills" class="rounded-lg p-3 mb-4 border"
                        :style="{
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select v-model="checkOutForm.paymentStatus" required
                                    ...
                                    :style="{
                                        ...
                                        borderColor: themeColors.border,
                                        color: themeColors.textPrimary,
                                        opacity: selectedGuest.hasUnpaidBills ? 0.7 : 1,
                                        cursor: selectedGuest.hasUnpaidBills ? 'not-allowed' : 'default'
                                    }">
                            ...
                            <p v-if="selectedGuest.hasUnpaidBills" class="text-xs mt-1"
                               :style="{ color: themeColors.textTertiary }">
                        ...
                        <div v-if="checkOutForm.paymentStatus === 'partial' && !selectedGuest.hasUnpaidBills">
                            <label class="block text-sm font-medium mb-2"
                                   :style="{ color: themeColors.textSecondary }">Outstanding Amount</label>
                ...
                <div class="flex items-center justify-end space-x-4 pt-6 border-t"
                    ...
                    <button type="submit" :disabled="isProcessing"
                            class="px-6 py-2 rounded-md transition-colors text-white"
                            :style="{
                                backgroundColor: isProcessing ? themeColors.border : themeColors.danger,
                                opacity: isProcessing ? 0.7 : 1
                            }"
                            @mouseenter="!isProcessing && ($event.target.style.backgroundColor = '#dc2626')"
                            @mouseleave="!isProcessing && ($event.target.style.backgroundColor = themeColors.danger)">
...
const processCheckOut = () => {
    ...
    router.post(route('front-desk.checkout.store'), {
        reservation_id: selectedGuest.value.id,
        ...
        onSuccess: () => {
            ...
            if (reservationId) {
                router.visit(route('front-desk.checkout.print', { reservation_id: reservationId }))
            }
        },
        onError: (errors) => {
            isProcessing.value = false

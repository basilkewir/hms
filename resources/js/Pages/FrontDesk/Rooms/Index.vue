        ...
        <div class="bg-kotel-dark border border-kotel-yellow/30 rounded-lg p-6 mb-8">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-kotel-sky-blue mt-2">View all rooms and manage check-ins/check-outs.</p>
                </div>
            </div>
        </div>
...
const quickCheckOut = (room) => {
    ...
    router.get(route('front-desk.checkout'), { reservation_id: room.reservation_id })
}

const quickCheckIn = (room) => {
    if (!room.pending_reservation) {
        selectedRoom.value = null
        router.get(route('front-desk.quick-checkin'), { room_id: room.id })
        return
    }
...
const manualCheckOut = (room) => {
    ...
    router.post(route('front-desk.rooms.manual-checkout', room.id), {}, {
        onSuccess: () => {
            selectedRoom.value = null
            statusType.value = 'success'
...
const markRoomClean = (room) => {
    ...
    router.post(route('front-desk.rooms.mark-clean', room.id), {}, {
        onSuccess: () => {
            selectedRoom.value = null
            statusType.value = 'success'

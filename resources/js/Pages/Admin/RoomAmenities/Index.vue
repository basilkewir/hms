                ...
                <form @submit.prevent="saveAmenity">
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center gap-2">
                            <input v-model="form.is_active" type="checkbox" id="is_active_admin" class="rounded" />
                            <label for="is_active_admin" class="text-sm font-medium" :style="{ color: themeColors.textSecondary }">Active</label>
                        </div>
                    </div>
...
const saveAmenity = () => {
    if (editingAmenity.value) {
        router.put(`/admin/room-amenities/${editingAmenity.value.id}`, form.value, { onSuccess: () => closeModal() })
    } else {
        router.post('/admin/room-amenities', form.value, { onSuccess: () => closeModal() })
    }
}

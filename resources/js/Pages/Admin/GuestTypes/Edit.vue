<template>
    <DashboardLayout title="Edit Guest Type" :user="user" :navigation="navigation">
        <!-- Header -->
        <div :style="{ backgroundColor: themeColors.card, borderColor: themeColors.border }" 
             class="shadow rounded-lg p-6 mb-8 border">
            <div class="flex items-center justify-between">
                <div>
                    <h1 :style="{ color: themeColors.textPrimary }" class="text-2xl font-bold">Edit Guest Type</h1>
                    <p :style="{ color: themeColors.textSecondary }" class="mt-2">Update guest type information.</p>
                </div>
...
const form = useForm({
    name: props.guestType.name,
    code: props.guestType.code || '',
    description: props.guestType.description || '',
    color: props.guestType.color || '#3B82F6',
    discount_percentage: props.guestType.discount_percentage || 0,
    is_active: props.guestType.is_active ?? true,
    sort_order: props.guestType.sort_order || 0,
})

const submit = () => {
    form.put(route('admin.guest-types.update', props.guestType.id))
}
</script>

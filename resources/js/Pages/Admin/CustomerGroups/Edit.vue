<template>
    <DashboardLayout title="Edit Customer Group" :user="user" :navigation="navigation">
        <!-- Header -->
        <div :style="{ backgroundColor: themeColors.card, borderColor: themeColors.border }" 
             class="shadow rounded-lg p-6 mb-8 border">
            <div class="flex items-center justify-between">
                <div>
                    <h1 :style="{ color: themeColors.textPrimary }" class="text-2xl font-bold">Edit Customer Group</h1>
                    <p :style="{ color: themeColors.textSecondary }" class="mt-2">Update customer group information.</p>
                </div>
...
const group = props.customerGroup || {}
const form = useForm({
    name: group.name || '',
    description: group.description || '',
    discount_percentage: group.discount_percentage != null ? parseFloat(group.discount_percentage) : 0,
    is_active: group.is_active ?? true,
})

const submit = () => {
    if (!props.customerGroup?.id) return
    form.put(route('admin.customer-groups.update', props.customerGroup.id))
}
</script>

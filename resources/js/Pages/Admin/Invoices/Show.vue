    ...
    <DashboardLayout title="Invoice Details" :user="user" :navigation="navigation">
        <div class="bg-white shadow rounded-lg p-6 mb-8" :style="{ backgroundColor: themeColors.background, borderColor: themeColors.border }">
            <div class="flex items-center justify-between">
                ...
                <Link :href="route('admin.invoices.index')" class="px-4 py-2 rounded-md font-medium text-white transition-colors"
...
const markAsPaid = () => {
    const form = useForm({})
    form.post(route('admin.invoices.markPaid', props.invoice.id), {
        onSuccess: () => {

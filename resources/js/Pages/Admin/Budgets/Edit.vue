<template>
    <DashboardLayout title="Edit Budget" :user="user">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-2xl font-bold mb-2"
                        :style="{ color: themeColors.textPrimary }">Edit Budget</h1>
                    <p class="mt-2"
                       :style="{ color: themeColors.textSecondary }">Update budget details and settings.</p>
                </div>
...
const submitForm = () => {
    ...
    router.put(route(`${props.routePrefix}.budget.update`, props.budget.id), form.value, {
        preserveScroll: true,
        onSuccess: () => {
        ...
        onError: (errors) => {
            processing.value = false
            console.error('Budget update errors:', errors)
        }
    })
}
</script>

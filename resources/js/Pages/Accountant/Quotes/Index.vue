<template>
    <DashboardLayout title="Quote Management" :user="user">
        <div class="space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2"
                       :style="{ color: themeColors.textSecondary }">Create and manage customer quotes.</p>
                </div>
                <div class="flex space-x-3">
                    <Link :href="route('accountant.quotes.create')" 
...
const applyFilters = () => {
    router.get(route('accountant.quotes.index'), filters.value, {
        preserveState: true,
        preserveScroll: true
    })
}
...
const viewQuote = (quote) => {
    router.get(route('accountant.quotes.show', quote.id))
}

const editQuote = (quote) => {
    router.get(route('accountant.quotes.edit', quote.id))
}

const sendQuote = (quote) => {
    if (confirm(`Send quote ${quote.quote_number} to ${quote.customer_email}?`)) {
        router.post(route('accountant.quotes.send', quote.id))
    }
}

const convertToInvoice = (quote) => {
    if (confirm(`Convert quote ${quote.quote_number} to invoice?`)) {
        router.post(route('accountant.quotes.convert', quote.id))
    }
}

const exportQuotes = () => {
    window.location.href = route('accountant.quotes.export', {
        ...filters.value,

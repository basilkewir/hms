                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <p class="text-xs mt-1"
                               :style="{ color: themeColors.textTertiary }">Or <Link :href="route('manager.guests.create')" :style="{ color: themeColors.primary }">create new guest</Link></p>
...
const onRoomChange = () => {
    ...
    if (form.room_id && selectedRoom.value) {
        ...
        if (roomType) {
            form.room_rate = roomType.base_price || 0
...
const onRoomRateChange = (index) => {
    const room = roomSelections.value[index]
    if (room.room_rate && room.selectedRoomType) {
        // Validate that custom rate is reasonable (optional)
        const baseRate = room.selectedRoomType.price
        if (room.room_rate < baseRate * 0.5) {
            // Optional: Show warning for very low rates
            console.log('Warning: Custom rate is significantly lower than base rate')
        }
    }
}
...
const submit = () => {
    ...
    form.post(route('manager.reservations.store'), {
        data: formData
    })
}
</script>

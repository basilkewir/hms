        ...
        <div class="bg-kotel-dark border border-kotel-yellow/30 rounded-lg p-6">
            ...
            <div v-if="viewMode === 'grid'" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
                <div v-for="room in filteredRooms" :key="room.id"
                    ...
                    <div v-if="room.housekeeping_status && room.housekeeping_status !== 'clean'"
                         class="absolute bottom-2 left-2">
                        <span class="px-2 py-1 rounded-full text-xs font-medium bg-amber-800/50 text-amber-300 border border-amber-400/50">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div class="bg-kotel-black/50 border border-kotel-yellow/20 rounded-lg p-3">
                                ...
                                <p class="text-white font-medium">{{ selectedRoom.last_cleaned || 'N/A' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="selectedRoom.status === 'available' && selectedRoom.pending_reservation">
                        ...
                        <div class="bg-kotel-black/50 border border-emerald-400/30 rounded-lg p-4">
                            ...
                            <div class="mt-2 grid grid-cols-2 gap-3 text-sm">
                                ...
                                <div><span class="text-kotel-sky-blue/90">Check-in</span><p class="text-white font-medium">{{ selectedRoom.pending_reservation.check_in_date }}</p></div>
                                <div><span class="text-kotel-sky-blue/90">Check-out</span><p class="text-white font-medium">{{ selectedRoom.pending_reservation.check_out_date }}</p></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="selectedRoom.reservation_id">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div class="bg-kotel-black/50 border border-kotel-yellow/20 rounded-lg p-3">
                                ...
                                <p class="text-white font-medium">{{ selectedRoom.check_in }}</p>
                            </div>
                            <div class="bg-kotel-black/50 border border-kotel-yellow/20 rounded-lg p-3">
                                <span class="text-kotel-sky-blue/90 text-sm block">Check-out</span>
                                <p class="text-white font-medium">{{ selectedRoom.check_out }}</p>
                            </div>
                            <div class="bg-kotel-black/50 border border-kotel-yellow/20 rounded-lg p-3">
...
const quickCheckOut = (room) => {
    ...
    router.get(route('manager.checkout'), { reservation_id: room.reservation_id })
}
...
const markRoomClean = (room) => {
    ...
    router.post(route('manager.rooms.mark-clean', room.id), {}, {
        onSuccess: () => {
            selectedRoom.value = null
            statusType.value = 'success'
...
const markRoomDirty = (room) => {
    ...
    router.post(route('manager.rooms.mark-dirty', room.id), {}, {
        onSuccess: () => {
            selectedRoom.value = null
            statusType.value = 'success'

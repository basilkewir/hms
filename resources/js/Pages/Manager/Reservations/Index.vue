<template>
    <DashboardLayout title="Reservations" :user="user">
        <!-- Header -->
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <Link :href="route('manager.reservations.arrivals')"
            ...
            <Link :href="route('manager.reservations.departures')"
                  class="rounded-lg p-4 transition-colors border"
                  :style="{ backgroundColor: themeColors.card, borderColor: themeColors.border }"
                  @mouseenter="$event.currentTarget.style.backgroundColor = themeColors.hover"
                  @mouseleave="$event.currentTarget.style.backgroundColor = themeColors.card">
            ...
            <Link :href="route('manager.room-assignment')"
                  class="rounded-lg p-4 transition-colors border"
                  :style="{ backgroundColor: themeColors.card, borderColor: themeColors.border }"
                  @mouseenter="$event.currentTarget.style.backgroundColor = themeColors.hover"
                  @mouseleave="$event.currentTarget.style.backgroundColor = themeColors.card">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full">
                    <thead :style="{ backgroundColor: themeColors.background }">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" :style="{ color: themeColors.textSecondary }">Confirmation</th>
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" :style="{ color: themeColors.textSecondary }">Guest</th>
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" :style="{ color: themeColors.textSecondary }">Dates</th>
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" :style="{ color: themeColors.textSecondary }">Room</th>
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" :style="{ color: themeColors.textSecondary }">Guests</th>
                    ...
                    <tbody>
                        <tr v-for="reservation in filteredReservations" :key="reservation.id"
                            ...
                            :style="hoveredRow === reservation.id ? { backgroundColor: themeColors.hover } : {}"
                            @mouseenter="hoveredRow = reservation.id"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    <Link :href="route('manager.reservations.show', reservation.id)" class="hover:opacity-80 transition-opacity" :style="{ color: themeColors.primary }">View</Link>
                                    <Link :href="route('manager.reservations.edit', reservation.id)" class="hover:opacity-80 transition-opacity" :style="{ color: themeColors.success }">Edit</Link>
...
const getStatusPillStyle = (status) => {
    ...
    if (key === 'cancelled') return { backgroundColor: themeColors.value.danger, color: '#fff' }
...
const checkIn = (reservation) => router.visit(`/manager/checkin?reservation_id=${reservation.id}`)
const checkOut = (reservation) => router.visit(`/manager/checkout?reservation_id=${reservation.id}`)
</script>

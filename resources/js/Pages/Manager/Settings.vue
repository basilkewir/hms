            ...
            <div class="p-6">
                ...
                <div v-show="activeTab === 'license'" class="space-y-6">
                    ...
                    <div v-else-if="licenseInfo" class="bg-kotel-dark border border-kotel-yellow rounded-lg p-6">
                        ...
                        <div class="flex space-x-3">
                            ...
                            <button @click="refreshLicenseInfo"
                            ...
                            <button @click="removeLicense"
                                    class="bg-red-600 text-white px-6 py-2 rounded-md hover:bg-red-700 font-medium transition-colors">
                                Remove License
                            </button>
                        </div>
                    </div>
                    ...
                    <div v-else class="bg-kotel-dark border border-red-500 rounded-lg p-6">
                        <div class="text-center">
                            ...
                            <p class="text-kotel-sky-blue mb-6">System requires a valid license to operate properly.</p>
                            <button @click="changeLicense"
                                    class="bg-kotel-yellow text-kotel-black px-6 py-3 rounded-md hover:bg-yellow-400 font-medium transition-colors">
                                Activate License
                            </button>
                        </div>
                    </div>
                ...
                <div v-show="activeTab === 'backup'" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <button class="bg-kotel-yellow text-kotel-black px-4 py-2 rounded-md hover:bg-kotel-yellow-dark font-medium transition-colors">
                                Run Backup Now
                            </button>
                        </div>
                    </div>
...
const settings = ref({
    ...
    theme_primary_color: props.settings?.theme?.theme_primary_color || '#0891ab', // Teal
    theme_secondary_color: props.settings?.theme?.theme_secondary_color || '#3b82f6', // Blue
    theme_background_color: props.settings?.theme?.theme_background_color || '#0b0b0b', // Black
    theme_sidebar_color: props.settings?.theme?.theme_sidebar_color || '#0f172a', // Dark Blue
    theme_card_color: props.settings?.theme?.theme_card_color || '#111827', // Dark Blue-Gray
    theme_text_primary: props.settings?.theme?.theme_text_primary || '#f3f4f6', // White
    theme_text_secondary: props.settings?.theme?.theme_text_secondary || '#9ca3af', // Gray
    theme_text_tertiary: props.settings?.theme?.theme_text_tertiary || '#6b7280', // Light Gray
    theme_border_color: props.settings?.theme?.theme_border_color || '#374151', // Gray
    theme_success_color: props.settings?.theme?.theme_success_color || '#22c55e', // Green
    theme_warning_color: props.settings?.theme?.theme_warning_color || '#f59e0b', // Orange
...
const loadLicenseInfo = async () => {
    ...
    try {
        ...
        if (response.ok) {
            licenseInfo.value = await response.json()
        }
    } catch (error) {
...
const removeLicense = async () => {
    if (confirm('Are you sure you want to remove the license? This will disable the system.')) {
        try {
            const response = await fetch('/api/license/remove', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            ...
            if (result.success) {
                ...
                notify.success('License removed successfully')
                // Reload the page to refresh license status throughout the system
                window.location.reload()
            } else {
                notify.error('Failed to remove license: ' + result.message)
            }
        } catch (error) {
            console.error('License removal error:', error)
            notify.error('Failed to remove license')
        }
    }
}
...
const resetTheme = () => {
    if (confirm('Are you sure you want to reset theme to default values?')) {
        ...
        settings.value.theme_primary_color = '#0891ab'

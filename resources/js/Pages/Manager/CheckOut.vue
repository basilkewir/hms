    ...
    <DashboardLayout title="Guest Check-Out" :user="user" :navigation="navigation">
        ...
        <div v-if="selectedGuest" class="shadow rounded-lg p-6"
            ...
            <form @submit.prevent="processCheckOut" class="space-y-6">
                <!-- Unified Final Bill -->
                <div>
                    <h4 class="text-md font-medium text-white mb-4">Unified Bill Summary</h4>
                    <div class="bg-kotel-black/50 border border-kotel-yellow/20 rounded-lg p-4 space-y-4">
                        <!-- Early Checkout Notice -->
                        <div v-if="selectedGuest.is_early_checkout" class="bg-yellow-500/20 border border-yellow-400/50 rounded-lg p-3 mb-4">
                            <div class="flex items-center">
                                <svg class="h-5 w-5 text-yellow-300 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                                <div>
                                    <p class="text-sm font-medium text-yellow-200">Early Checkout Detected</p>
                                    <p class="text-xs text-yellow-300/90 mt-1">
                            ...
                            <div class="space-y-2 mb-2">
                                <div v-for="sale in selectedGuest.posSales" :key="sale.id" 
                                    ...
                                    <div class="flex justify-between items-center px-2 py-1.5 border-b border-kotel-yellow/10">
                                        ...
                                        <span class="text-[11px] text-gray-400">{{ sale.sale_date }}</span>
                                    </div>
                        ...
                        <div class="pt-4 border-t-2 border-kotel-yellow/30 space-y-2 text-gray-300">
                            ...
                            <div class="flex justify-between font-bold text-lg text-white">
                                ...
                                <span>{{ formatCurrency(parseFloat(selectedGuest.unifiedTotal?.replace(/,/g, '') || selectedGuest.totalWithServices?.replace(/,/g, '') || 0)) }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span>Paid Amount</span>
                                <span>{{ formatCurrency(parseFloat(selectedGuest.paidAmount?.replace(/,/g, '') || 0)) }}</span>
                            </div>
                            <div v-if="parseFloat(selectedGuest.unifiedBalance?.replace(/,/g, '') || selectedGuest.balanceWithServices?.replace(/,/g, '') || 0) > 0" 
                                 class="flex justify-between text-red-300 font-semibold mt-2 pt-2 border-t border-kotel-yellow/20">
                                <span>Balance Due</span>
                                <span>{{ formatCurrency(parseFloat(selectedGuest.unifiedBalance?.replace(/,/g, '') || selectedGuest.balanceWithServices?.replace(/,/g, '') || 0)) }}</span>
                            </div>
...
const props = defineProps({
    user: Object,
    todaysDepartures: { type: Array, default: () => [] },
    allCheckedIn: { type: Array, default: () => [] },
    selectedReservationId: { type: [Number, String], default: null },
})
...
if (id && (props.todaysDepartures?.length || props.allCheckedIn?.length)) {
...
const processCheckOut = () => {
    ...
    router.post(route('manager.checkout.store'), {
        ...
        onSuccess: () => {
            ...
            if (reservationId) {
                router.visit(route('manager.checkout.print', { reservation_id: reservationId }))
            }
        },
        onError: (errors) => {
            isProcessing.value = false
...
const getBalanceColor = (balance) => {
    ...
    if (balanceAmount > 0) return themeColors.value.danger
    if (balanceAmount < 0) return themeColors.value.success
    return themeColors.value.textPrimary
}
...
const formatTime = (time) => {
    if (!time) return 'N/A'
    if (typeof time === 'string' && time.includes('T')) return new Date(time).toLocaleDateString()
    return time
}
</script>

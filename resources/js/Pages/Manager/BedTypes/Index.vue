<template>
    <DashboardLayout title="Bed Types" :user="user" :navigation="navigation">
            ...
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-kotel-yellow"
                        style="color: #FFD700;">Bed Types Management</h1>
                    <p class="text-kotel-sky-blue mt-1"
                       style="color: #87CEEB;">Manage bed types and their specifications</p>
                </div>
                ...
                <table class="min-w-full divide-y divide-kotel-yellow/20"
                    ...
                    <thead class="bg-kotel-black/50"
                        ...
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-kotel-sky-blue uppercase tracking-wider"
                                style="color: #87CEEB;">Code</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-kotel-sky-blue uppercase tracking-wider"
                                style="color: #87CEEB;">Dimensions</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-kotel-sky-blue uppercase tracking-wider"
                                style="color: #87CEEB;">Description</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-kotel-sky-blue uppercase tracking-wider"
                                style="color: #87CEEB;">Rooms</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-kotel-sky-blue uppercase tracking-wider"
                                style="color: #87CEEB;">Room Types</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-kotel-sky-blue uppercase tracking-wider"
                    ...
                    <tbody class="bg-kotel-black/30 divide-y divide-kotel-yellow/20"
                        ...
                        <tr v-for="bedType in bedTypes" :key="bedType.id" 
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-white"
                                style="color: #ffffff;">{{ bedType.name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-kotel-sky-blue/80"
                                style="color: rgba(135, 206, 235, 0.8);">{{ bedType.code || '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-kotel-sky-blue/80"
                                style="color: rgba(135, 206, 235, 0.8);">
                                <span v-if="bedType.width_inches && bedType.length_inches">
                                    {{ bedType.width_inches }}" × {{ bedType.length_inches }}"
                                </span>
                                <span v-else>-</span>
                            </td>
                            <td class="px-6 py-4 text-sm text-kotel-sky-blue/60"
                                style="color: rgba(135, 206, 235, 0.6);">{{ bedType.description || '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-kotel-sky-blue/80"
                                style="color: rgba(135, 206, 235, 0.8);">{{ bedType.room_count }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-kotel-sky-blue/80"
                                style="color: rgba(135, 206, 235, 0.8);">{{ bedType.room_type_count }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full border" 
                                      :style="bedType.is_active ? {
                                    ...
                                    {{ bedType.is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <Link :href="route('admin.bed-types.edit', bedType.id)" 
                                      class="text-kotel-yellow hover:text-kotel-yellow/80 mr-3 transition-colors"
                                      style="color: #FFD700;">Edit</Link>
                                <button @click="deleteBedType(bedType)" 
...
const deleteBedType = (bedType) => {
    if (confirm(`Are you sure you want to delete ${bedType.name}?`)) {
        router.delete(route('admin.bed-types.destroy', bedType.id))
    }
}
</script>

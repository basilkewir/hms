                        ...
                        <tr v-for="task in tasks?.data || []" :key="task?.id"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    <Link :href="`/manager/housekeeping-tasks/${task.id}`"
                                          class="transition-colors"
...
const deleteTask = (task) => {
    if (confirm('Are you sure you want to delete this task?') && task?.id) {
        router.delete(`/manager/housekeeping-tasks/${task.id}`)
    }
}
...
const generateTasks = () => {
    ...
    router.post(route('manager.housekeeping-tasks.generate-daily'), {}, {

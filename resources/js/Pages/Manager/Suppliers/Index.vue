...
const saveSupplier = () => {
  if (showEditModal.value && editingSupplier.value) {
    router.put(`/manager/suppliers/${editingSupplier.value.id}`, form.value, {
      onSuccess: closeModal
    })
  } else {
    router.post('/manager/suppliers', form.value, {
      onSuccess: closeModal
    })
  }
}
...
const viewOrders = (supplier) => {
  router.visit(`/manager/purchases?supplier_id=${supplier.id}`)
}
</script>

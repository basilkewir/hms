    ...
    <DashboardLayout title="Supplier Payments" :user="user" :navigation="navigation">
        <div class="mb-6 flex justify-between items-center">
            <div>
                ...
                <Link :href="route('pos.suppliers.show', supplier.id)" class="text-kotel-sky-blue hover:text-kotel-yellow">
                    ← Back to Supplier
                </Link>
            </div>
            <button
...
const savePayment = () => {
    router.post(route('pos.suppliers.payments.store', props.supplier.id), paymentForm, {
        preserveScroll: true,
        onSuccess: () => {
...
const deletePayment = (payment) => {
    if (confirm(`Are you sure you want to delete payment ${payment.payment_number}?`)) {
        router.delete(route('pos.supplier-payments.destroy', payment.id), {
            preserveScroll: true
        });
    }
};

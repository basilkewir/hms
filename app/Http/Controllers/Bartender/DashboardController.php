...
{
    /**
     * Display the bartender dashboard with real data.
     */
    public function index()
    {
        $user = Auth::user()->load('roles');

        // Get bar-related products (drinks, beverages, cocktails)
        ...
        // Get food-related products if restaurant is enabled
        ...
        // Get available food items with details
        $availableFoodItems = Product::where('is_active', true)
            ->whereIn('category_id', [
                DB::table('product_categories')->where('name', 'LIKE', '%food%')->value('id') ?? 0,
                DB::table('product_categories')->where('name', 'LIKE', '%restaurant%')->value('id') ?? 0,
                DB::table('product_categories')->where('name', 'LIKE', '%appetizer%')->value('id') ?? 0,
            ...
            ->map(function ($food) {
                return [
                    ...
                    'min_stock_level' => $food->min_stock_level,
                    'emoji' => $food->emoji ?? '🍽️',
                ];
            });
